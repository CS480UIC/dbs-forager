...
CREATE INDEX receiver_type_idx ON receiver (receiver_type);

CREATE INDEX donor_type_idx ON donor(donor_type);

CREATE INDEX location_idx ON volunteer(location);

CREATE INDEX address_idx ON users(address);

CREATE INDEX idx_foodType ON food (food_type);

CREATE INDEX idx_order ON  food_order (order_id, food_id);

CREATE INDEX index_1 ON restaurant (safety_rating);

CREATE UNIQUE INDEX index_2 ON interaction (to_id,from_id,date_created);
